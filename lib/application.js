...
Application.prototype.run = function(callback) {
    ...
    this._alarm.on('ring', (function() {
        ...
        this._radio.isPlaying((function(err, isPlaying) {
            if (err) {
            ...
            this._getAlarmStation((function(err, station) {
                if (err) {
                    return console.error(err);
                }
...
Application.prototype._getAlarmStation = function(callback) {
    this._settings.get('alarm_station', (function(err, stationId) {
        if (err) {
            return callback(err);
        }

        this._stationsKeeper.get(stationId, (function(err, station) {
            if (err) {
                return callback(err);
            }
