 ...
 */
Radio.prototype.play = function(station, callback) {
    // Update current radio station if need
    if (station) {
        try {
            this.setCurrentStation(station);
        } catch(err) {
            // By some reasons we could not set the current station
            return callback(err);
        }
    }
 ...
 * object. Otherwise it will be set to null.
 */
Radio.prototype.fadeIn = function(station, callback) {
    var self = this;
    ...
    this._mpdPool.getClient(function(err, client) {
        ...
        client.sendCommand(
            ...
            function() {
                // Turn the radio on
                self.play(station, function(err) {
                    if (err) {
                        return callback(err);
                    }
