...
module.exports = function(config, callback) {
    ...
    /**
     * Return MPD client with alive connection.
     *
    ...
    var getMpdClient = (function() {
        ...
        return function(cb) {
            if (mpdClient) {
                ...
                // Try to establish new connection to MPD.
                mpdClient = mpd.connect({
                   host: config.mpdHost,
                   port: config.mpdPort
                });

                // We need to remove MPD client when connection is closed.
                mpdClient.on('end', function() {
                    mpdClient.removeAllListeners();
                    mpdClient = null;
                });

                // Update internal "isPlaying" value when mpd state changed
                ...
                // Return client instance via callback.
    ...
    getMpdClient(function(client) {
        client.sendCommand(mpd.cmd('setvol', [100]));
